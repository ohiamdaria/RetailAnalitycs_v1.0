...
-- DROP MATERIALIZED VIEW base CASCADE;
-- CREATE MATERIALIZED VIEW base AS (SELECT DISTINCT support_view.customer_id,
--                                 support_view.group_id,
--                                 support_view.transaction_id AS transaction_id,
--                                 support_view.transaction_datetime AS Transaction_DateTime
--                 FROM support_view INNER JOIN cards ON support_view.customer_id = cards.customer_id
--                 INNER JOIN periods p on (cards.customer_id = p."Customer_ID" AND p."Group_ID" = support_view.group_id)
--                 WHERE support_view.transaction_datetime >= p."First_Group_Purchase_Date"
--                       AND
--                       support_view.transaction_datetime <= p."Last_Group_Purchase_Date"
--                 ORDER BY 1, 2, 4);


DROP MATERIALIZED VIEW groups CASCADE;
